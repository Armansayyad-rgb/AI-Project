...
BASE_MODEL_FILE = Path(
    r"C:\AI-Project\checkpoints\v2\instruction_model_v4.pt"
)
...
def build_training_example(
    ...
    # Remove automatic special tokens.
    prompt_ids = [
        token_id
    ...
    # Reserve room for answer and EOS.
    max_prompt_length = (
        MAX_LENGTH
        - len(answer_ids)
        - 2
    ...
    # Keep the end because Question/Answer
    # markers appear there.
    ...
    # Ignore all prompt tokens in the loss.
    for i in range(
        min(
            answer_start - 1,
            len(labels),
...
def main():
    ...
    print(
        "V4 extractive fine-tuning complete."
    )
